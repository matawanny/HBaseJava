...
round(pc_issuance_note_rate,3) as pc_issuance_note_rate, round(net_note_rate,2) as net_note_rate, orig_loan_amt, orig_upb, loan_age, rem_months_to_maturity, 
...
NVL(prefix, '') as prefix, IF(ins_cancel_ind='7', '', ins_cancel_ind) as ins_cancel_ind, NVL(govt_ins_grnte, '') as govt_ins_grnte, 
...
round(origin_note_rate,2) as origin_note_rate, round(origin_loan_amt,2) as origin_loan_amt, 
...
from fhlmc_mod_loan_daily where as_of_date=$asOfDateMonthly" -o $output_file --print_header --output_delimiter=\|
